...
Recent research has focused on translating Behavior Trees into more formal state-based representations to enable verification and performance analysis. The BT2Automata framework introduces Timed Behavior Trees and provides a translation to timed automata, allowing temporal properties such as timing constraints and execution deadlines to be checked using tools like UPPAAL \cite{bt2automata}. Similarly, work on Probabilistic Timed Behavior Trees extends this formulation by mapping BTs to probabilistic timed automata, enabling reasoning about stochastic reliability and task performance under uncertainty \cite{ptbt}. Another approach, BT2Fiacre, compiles BTs into the Fiacre specification language, which is then transformed into timed transition systems for model checking and reachability analysis \cite{bt2fiacre}.

These methods demonstrate that converting BTs into formal automaton-like models provides a foundation for systematic verification. However, they largely focus on single-robot analysis and temporal fidelity, rather than explicitly modeling interactions between multiple robots. By contrast, our work targets multi-robot safety verification by converting each robot’s Behavior Tree into a finite-state machine, constructing their combined product state space, and performing explicit unsafe-state reachability analysis to detect and correct dangerous interactions between concurrent robot behaviors.